...
from afigan.engine import transform_gen as T
...
def build_transform_gen_ratio(cfg, ratio,is_train):
    """
    Create a list of :class:`TransformGen` from config.
    Now it includes resizing and flipping.
    ...
    """
    if is_train:
        min_size = tuple([x*ratio for x in cfg.INPUT.MIN_SIZE_TRAIN])
        max_size = cfg.INPUT.MAX_SIZE_TRAIN * ratio
        sample_style = cfg.INPUT.MIN_SIZE_TRAIN_SAMPLING
    else:
        min_size = cfg.INPUT.MIN_SIZE_TEST * ratio
        max_size = cfg.INPUT.MAX_SIZE_TEST * ratio
        sample_style = "choice"
    if sample_style == "range":
        assert len(min_size) == 2, "more than 2 ({}) min_size(s) are provided for ranges".format(
            len(min_size)
    ...
    logger = logging.getLogger(__name__)
    tfm_gens = []
    tfm_gens.append(T.Resize_same_ratio(ratio))
    tfm_gens.append(T.ResizeShortestEdge(min_size, max_size, sample_style))
    if is_train:
        tfm_gens.append(T.RandomFlip())
        logger.info("TransformGens used in training: " + str(tfm_gens))
...
def build_transform_gen_ratio2(cfg, ratio, prev_tfm_gens, is_train):
    """
    Create a list of :class:`TransformGen` from config.
    Now it includes resizing and flipping.
...
def build_transform_gen_noflip(cfg, is_train):
    """
    Create a list of :class:`TransformGen` from config.
    Now it includes resizing and flipping.
