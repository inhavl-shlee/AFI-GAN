...
__all__ = [
    ...
    "Resize_same_ratio",
    "apply_transform_gens_overlap",
    "apply_transform_gens_overlap2"
]
...
#######################CUSTOM
class Resize_same_ratio(TransformGen):
    """ Resize image to a target size"""

    def __init__(self, ratio, interp=Image.BILINEAR):
        """
        Args:
            shape: (h, w) tuple or a int
            interp: PIL interpolation method
    ...
    def get_transform(self, img):
        return ResizeTransform(
            img.shape[0], img.shape[1], int(img.shape[0]*self.ratio), int(img.shape[1]*self.ratio), self.interp
        )
...
import copy
def apply_transform_gens_overlap(transform_gens, img, Trans):
    """
    Apply a list of :class:`TransformGen` on the input image, and
    returns the transformed image and a list of transforms.
    ...
    tfms = []
    for idx, g in enumerate(transform_gens):
        tfm = g.get_transform(img)
        if idx==2:
            tt = copy.deepcopy(Trans.transforms[-1])
            if isinstance(tt, T.HFlipTransform):
                tt.width = int(tt.width * 0.5)
            if isinstance(tt, T.VFlipTransform):
                tt.height = int(tt.height * 0.5)
...
def apply_transform_gens_overlap2(transform_gens, img, Trans):
    """
    Apply a list of :class:`TransformGen` on the input image, and
    returns the transformed image and a list of transforms.
    ...
    for idx, g in enumerate(transform_gens):
        tfm = g.get_transform(img)
        if isinstance(tfm,T.ResizeTransform):
            tt = Trans.transforms[0]
            tfm.new_h = int(tt.new_h*0.5)
            tfm.new_w = int(tt.new_w * 0.5)

        if isinstance(g,T.RandomFlip):
            tt = copy.deepcopy(Trans.transforms[-1])
            if isinstance(tt, T.HFlipTransform):
                tt.width = int(tt.width * 0.5)
            if isinstance(tt, T.VFlipTransform):
